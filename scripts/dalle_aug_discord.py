...
def save_quantized(model: BaseAWQForCausalLM, save_dir: str, shard_size="10GB"):
    ...
    shards, index = shard_checkpoint(
        model.model.state_dict(), max_shard_size=shard_size, weights_name=model_name
    )
...
def main(
    ...
    if not quantized_ckpt_path.exists() or force_quantize:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            language_model_id, trust_remote_code=True
        )
    ...
    diffusion_pipeline = DiffusionPipeline.from_pretrained(
        diffusion_model_id, variant="fp16"
    )

    print("Loading LCM lora...")
    diffusion_pipeline.load_lora_weights(diffusion_lora_id)
    diffusion_pipeline.scheduler = LCMScheduler.from_config(
    ...
    async def on_message(message: discord.Message):
        ...
        if message.channel.id not in allowed_channels:
            return
        messages = dalle_conversation(message.content)
        templated_messages: str = tokenizer.apply_chat_template(messages, tokenize=False)  # type: ignore
        t0 = time.perf_counter()
        outputs = vllm_model.generate(
            templated_messages, sampling_params=sampling_params
        )
        ...
        generated_image: Image.Image = diffusion_pipeline(
            prompt=outputs_text, num_inference_steps=4, guidance_scale=1
        ).images[0] # type: ignore
        with BytesIO() as image_binary:
            generated_image.save(image_binary, "PNG")
            image_binary.seek(0)
            await message.channel.send(
                outputs_text, file=discord.File(fp=image_binary, filename="image.png")
            )
        print(outputs_text)
